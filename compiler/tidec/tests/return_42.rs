//! Integration test: program that returns 42.
...
/// Test that a program returning 42 compiles and runs correctly.
#[test]
fn test_return_42() {
    let runner = TestRunner::new("return_42");

    let test_ctx = TestContext::new();
    let intern_ctx = InternCtx::new(&test_ctx.arena);
    let tir_ctx = TirCtx::new(&test_ctx.target, &test_ctx.arguments, &intern_ctx);

    let tir_unit = create_return_42(&tir_ctx);
    runner.compile(tir_ctx, tir_unit);

    runner.link().expect("Linking failed");
    assert_eq!(runner.run(), Some(42), "Expected exit code 42");
}
